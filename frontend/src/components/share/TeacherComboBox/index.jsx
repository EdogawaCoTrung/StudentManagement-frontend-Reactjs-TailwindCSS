...
export default function TeacherComboBox({ handleChange, subjectId, teacher, teacherId }) {
  ...
  const handleChangeInput = (e) => {
    setSelected(e)
    handleChange("teacherId", e.id)
  }
  const [query, setQuery] = useState("")
  const [cloneQuery, setCloneQuery] = useState("")
  const [teachers, setTeachers] = useState("")
  const fetchAllTeacher = async () => {
    let res = await teacherApi.getAllTeacherBySubjectId(subjectId)
    if (res.EC == 1) {
      toast.error(res.EM)
    }
    //   console.log()
            ...
            (query.includes("-")
              ? cloneQuery.split("-")[1].includes(teacherDT.id.toString()) &&
                query
                  ...
                  .replace(/[\u0300-\u036f]/g, "")
                  .replace(/\s+|-/g, "")
                  .includes(teacherDT.teachername.toLowerCase().replace(/\s+/g, ""))
              : query
                  .toLowerCase()
                  .normalize("NFD")
                  .replace(/[\u0300-\u036f]/g, "")
                  .replace(/\s+|-/g, "")
                  .includes(
                    teacherDT.teachername.toLowerCase().normalize("NFD").replace(/\s+|-/g, "").replace(/\s+/g, ""),
                  )),
        )
  return (
          ...
          <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
            <Combobox.Input
              ...
              displayValue={(teachers) =>
                teachers.id === "" ? teachers.teachername + teachers.id : teachers.teachername + " - " + teachers.id
              }
              onChange={(event) => {
                    ...
                    {({ selected, active }) => (
                      <>
                        <span className={`block truncate ${selected ? "font-medium" : "font-normal"}`}>
                          {teacherDT.teachername} - {teacherDT.id}
                        </span>
                        {selected ? (
                          <span
